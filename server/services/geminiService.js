...
const generateIdea = async (domain, skillLevel, teamSize, purpose) => {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            ...
            // Use Groq chat completion when available
...
- If the question is unrelated to the project, politely redirect to the project context`;
    ...
    // Build a single prompt combining the system context, chat history, and the user's message.
    let combined = systemContext + '\n\nPlease acknowledge that you understand this project and are ready to help.\n\n';

    if (chatHistory && chatHistory.length > 0) {
        for (const msg of chatHistory) {
            const role = msg.role === 'user' ? 'User' : 'Assistant';
            combined += `${role}: ${msg.content}\n`;
        }
        combined += '\n';
    }
    ...
    try {
        ...
        const response = await runCompletion(combined);
...
// Helper: run a text completion using Groq SDK (preferred). Returns the resulting text.
async function runCompletion(prompt) {
    // Prefer Groq client when available
    try {
        if (groq && groq.chat && groq.chat.completions && typeof groq.chat.completions.create === 'function') {
            const modelName = process.env.GROQ_MODEL || 'llama-3.3-70b-chat';
            const res = await groq.chat.completions.create({
                model: modelName,
                messages: [
                    { role: 'user', content: prompt }
                ],
                // allow caller to control tokens via env if needed
            ...
            if (res?.choices && res.choices.length > 0) {
                ...
                if (choice.message && (choice.message.content || choice.message.role)) {
                    return choice.message.content || JSON.stringify(choice.message);
                }
                if (choice.text) return choice.text;
                if (choice.output) return JSON.stringify(choice.output);
            }
        ...
        err.message = `Groq completion failed: ${err.message}`;
        throw err;
    }

    throw new Error('No supported AI client configured. Set GROQ_API_KEY and install groq-sdk.');
}
